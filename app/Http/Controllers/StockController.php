...
class StockController extends Controller
{
    ...
    public function post(Request $request){
        ...
        if($checkexist == null){
            ...
            $countrows = sizeof($request->nama_kartu_d);
            $count=0;
            for ($i=0;$i<sizeof($request->nama_kartu_d);$i++){
                Stock_d::create([
                    'idh' => $idh,
    ...
    public function getedit(Stock $stock){
        // dd($stock);
        $stockds = stock_d::select('id','idh','nama','quantity','satuan')->where('id','=',$stock->idh)->get();
        $barangs = Card::all();
    ...
    public function update(Stock $stock){
        // dd(request()->all());
        // dd($penjualan->id);
        DB::delete('delete from stock_ds where id = ?', [$stock->idh] );
    ...
    public function delete(Stock $stock){
        ...
        return redirect()->route('stocklist')->with('success', 'Data berhasil di Delete');
    }
}
